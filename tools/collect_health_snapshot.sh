...
append_cmd "resetprop | grep -E 'cached_apps_freezer|fpsgo|perf_turbo|smart_ram_management|boot.logcat|debug\\.sf.disable_backpressure|debug\\.sf.latch_unsignaled|force_highend_gfx|composition.type|powermode|logcat.live'"

write_section "Tunables"
append_cmd "for f in /proc/sys/vm/swappiness /proc/sys/vm/vfs_cache_pressure /proc/sys/vm/dirty_ratio /proc/sys/vm/dirty_background_ratio /proc/sys/vm/compaction_proactiveness /proc/sys/vm/watermark_scale_factor /proc/sys/vm/page-cluster /proc/sys/vm/dirty_writeback_centisecs /sys/kernel/mm/lru_gen/enabled /sys/devices/system/cpu/cpufreq/sugov_ext/up_rate_limit_us /sys/devices/system/cpu/cpufreq/sugov_ext/down_rate_limit_us; do [ -e \"\$f\" ] && printf '%s=%s\n' \"\$f\" \"\$(cat \$f 2>/dev/null)\"; done"
...
append_cmd "ps -A -o PID,NAME,ARGS 2>/dev/null | grep -E 'facebook|instagram|youtube|wps|twitter|supplicant|surfaceflinger|thermal' | grep -v grep | sed -n '1,120p'"
