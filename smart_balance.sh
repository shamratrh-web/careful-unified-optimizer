#!/system/bin/sh
# Careful Unified Optimizer v6.2 Gaming Pro - Live Balance
...
if [ "$current_mode" != "$target_mode" ] || [ "$target_mode" = "turbo" ]; then
    case "$target_mode" in
        turbo)
            apply_turbo_mode
            ;;
...
log_perfmgr "v6.2 mode=$target_mode cpu=${CAREFUL_CPU_TEMP} bat=${CAREFUL_BATTERY_TEMP}"
